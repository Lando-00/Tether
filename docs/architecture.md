...
`NotebookOrchestrator` implements [ADR-0020](./adr/0020-notebook-orchestrator-algorithm.md)'s Hanov-style five-phase loop for explicit research requests: **Plan → Explore → Extract → Refine → Synthesize**. It keeps only structured notebook facts between searches, then streams the final synthesized answer as normal `text_delta` events. Research turns use the normal persisted lifecycle: history is read before the exact raw prompt is stored, then the assistant answer and completed turn are persisted.

Research mode is opt-in per request (`mode="research"` or the CLI research mode). Keep `research: "tether.protocol.orchestration.notebook.NotebookOrchestrator"` under `orchestrator.registry` and `web_search` in `tools.enabled`; `Engine.from_settings` fails fast if research is registered without `web_search`. Short whole-message corrections ending in `*` are reconstructed only for the current research turn; the stored transcript remains literal. Ambiguous corrections and detected near-spelling entity drift stop before search and ask for clarification.

The v2 research vocabulary includes phase start/progress, fact, query, limit, no-facts, and clarification events. `NotebookClarificationRequested` is terminal in the order `message_start` → clarification → `message_stop(complete)` and does not invoke a model or tool. `NotebookFactAdded.source_kind` distinguishes `web_search` from `local_deterministic`: narrow finite-decimal arithmetic is calculated locally with `Decimal`, never searched, and a pure calculation skips Plan/Explore/Extract.

Planner output, extractor follow-ups, and the empty-plan residual fallback share a bounded query sanitizer. The parser retains its five layers; its candidate scan is limited to 64 KiB, 16 objects, and depth 64. `orchestrator.research.synth_assume_open_think_models` is an exact-model-ID opt-in for streams known to begin in hidden `<think>` content. Listed models fail closed with non-fatal `UnclosedThinkBlock` if no close marker arrives; non-listed models retain first-token streaming, including the accepted long-hidden-preamble edge case. Abandoned async-generator cleanup is tracked in a bounded process tracker surfaced as informational-only `/readyz` `operational_health.notebook_cleanup`; it never affects readiness or initiates provider recovery.
