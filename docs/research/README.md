...
| [`10_xllamacpp_experimentation.md`](./10_xllamacpp_experimentation.md) | **xllamacpp / Vulkan + Adreno experiment (May 2026).** Native ARM64 Vulkan wheel installs cleanly and detects the Adreno X1 GPU. Engine load is 10× faster than MLC (3 s vs 30 s). **But:** Vulkan/Adreno backend produces garbage tokens at every prompt size (driver-level), and the CPU ARM64 fallback only stays coherent for prompts ≤ ~50 tokens before also producing gibberish (regardless of model source or REPACK toggle). Wheel was added 3 days before the experiment (xllamacpp #137). Not viable for Tether today; revisit in 3-6 months. |
| [`11_rlm_orchestration.md`](./11_rlm_orchestration.md) | **Recursive Language Models (RLMs) — orchestration research (Aug 2026).** Primary-source review of [arXiv:2512.24601](https://arxiv.org/abs/2512.24601) (Zhang, Kraska, Khattab; MIT OASYS) — context offloaded into a REPL variable that the root LM programmatically slices and recursively sub-queries. Strong published results, but **not adoptable here**: the root model must write reliable Python (smallest tested is GPT-5-mini, vs our Qwen3-1.7B), latency lands at ~2–10 min/query at 20 tok/s, and the default REPL is an in-process `exec()`. **Verdict: adapt, don't adopt** — the context-as-variable and per-fragment extraction ideas map directly onto the scoped "Option C" fact-extraction work, no REPL required. |
